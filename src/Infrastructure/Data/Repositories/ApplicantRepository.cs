...
namespace ApplicantPortal.Infrastructure.Data.Repositories;

public class ApplicantRepository(IApplicationDbContext context, IMapper mapper) : IApplicantRepository
{
    ...
    {
        return await context.ConfigurationModels.OrderByDescending(b => b.Id).FirstOrDefaultAsync();
    }
    ...
    public int GetTotalAggregate(List<int> Cores, List<int> CoreAlt, List<int> Electives)
    {
        ...
        var SliceCoreAlt = CoreAlt.Skip(CstartIndex).Take(Clenght) ??
                           throw new ArgumentNullException("Cores");
    ...
    public int GetGrade(string applicant)
    {
        ...
        return data != null ? (int)data.Grade! : 0;
    }
    ...
    public async Task<int> UpdateFormNo(CancellationToken cancellationToken)
    {
        ...
        var update = context.FormNoModels.First(n => configuration != null && n.Year == configuration.Year);
        update.No += 1;
    ...
    {
        var data = await context.DistrictModels.ToListAsync(cancellationToken: cancellationToken);
    ...
    {
        var data = await context.ShsProgrammes.ToListAsync(cancellationToken: cancellationToken);
    ...
    {
        var data = await context.Languages.ToListAsync(cancellationToken: cancellationToken);
    ...
    {
        var data = await context.ShsAttendedModels.ToListAsync(cancellationToken: cancellationToken);
    ...
    {
        var data = await context.UniversityAttendedModels.ToListAsync(cancellationToken: cancellationToken);
